...
(function() {
  ...
  NotesController.prototype.initializeProcessNotes = function(scrollPosition) {
    ...
    if (self.showProcessNotes) {
      self.notesService.getProcessNotes(self.$scope.processInstanceOid).then(function(data) {
        // find only relevant notes
        data.data.list = classifyProcessNotes(data.data.list, self.$scope.processInstanceOid);
        data.data.totalCount = data.data.list.length;
   ...
   * classify notes
   */
  function classifyProcessNotes(notesList, pOid) {
    var processNotes = [];
    if (!notesList) { return processNotes }

    for ( var noteInd in notesList) {
      if (pOid == notesList[noteInd].contextOID) {
        processNotes.push(notesList[noteInd]);
      }
    }
    return processNotes;
  }
